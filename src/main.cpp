        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for(int i=0; i< sensor_fusion.size(); i++)
            {
              ...
              {
                double vx = sensor_fusion[i][3]; //vx
                double vy = sensor_fusion[i][4]; //vy
                double check_speed = sqrt(vx*vx + vy*vy); //speed of car in my lane 
                double check_car_s = sensor_fusion[i][5]; //s

                //Predit where a car will be in the future
                check_car_s += ((double) prev_size* .02*check_speed);
                if(debug)
                {
                  cout << "my lane " << lane << " "<< i <<" d=" << d <<endl;
                  cout << "check_car_s - car_s: " << (check_car_s - car_s) << endl;
                }
                //check s values greater than mine means it's ahead of me and 
                //s gap is lower then 30
                if((check_car_s > car_s)&&((check_car_s - car_s) < 30))
                {
                  ...
                  //lane change --> leads to the change in waypoints in spline
                  //if(lane > 0)
                  //{
                  //  lane = 0;
                  //}
              ...
              if((d < (4*lane) && d > 4*(lane-1))) //left of me
              {
                double vx = sensor_fusion[i][3]; //vx
                double vy = sensor_fusion[i][4]; //vy
                double check_speed = sqrt(vx*vx + vy*vy); //speed of car in my lane 
                ...
                if(((car_s -  check_car_s) <8) && ((check_car_s - car_s) < 35))
                {
                  left_available = false;
                }
              }
              if((d < 4*(lane+2) && d > 4*(lane+1))) //right of me
              {
                double vx = sensor_fusion[i][3]; //vx
                double vy = sensor_fusion[i][4]; //vy
                double check_speed = sqrt(vx*vx + vy*vy); //speed of car in my lane 
              ...
              if (debug)
              {
                cout << "left_available: " << left_available <<" right_available: " << right_available<<endl;
              }
            }
            // lane change or not
            if((too_close || close) && left_available)
            {
              if(lane > 0)
              {
                lane -= 1;
            ...
            else if((too_close || close) && right_available)
            {
              if(lane<2)
              {
                lane += 1;
            ...
            // reference x, y, yar states
            // either we will reference the starting pint as where the car is or 
            // at the previous paths end point
            double ref_x = car_x;
            double ref_y = car_y;
            double ref_yaw = deg2rad(car_yaw);

            // if prevous size is almost empty, use the car as starting reference
            if(prev_size<2)
            {
            ...
            {
              //Redefine refeence state as previous path end point
            ...
            s.set_points(ptsx, ptsy); //add anchor points to the line
